class Relation
  ...
  def bcnf_decomposition
    ...
    attrs = functional_dependency_set.closure(@bcnf_violating_fd.determinant.join(''))
    fds = functional_dependency_set.related_to(attrs.chars.to_a)
    r1 = Relation.new attrs, fds

    attrs = (attributes - attrs.chars.to_a + @bcnf_violating_fd.determinant).sort.uniq
    fds = functional_dependency_set.related_to(attrs)
    r2 = Relation.new attrs.join(''), fds
